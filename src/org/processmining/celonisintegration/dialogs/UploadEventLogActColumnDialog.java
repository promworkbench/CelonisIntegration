...
public class UploadEventLogActColumnDialog {
	public UploadEventLogActColumnDialog(UIPluginContext context, final UploadEventLogParameter parameters, XLog log) {
        ...
        int w = JOptionPane.showConfirmDialog(null, myPanel,
                "Choose the activity column",JOptionPane.OK_CANCEL_OPTION);

        context.getFutureResult(0).setLabel("Activity column");
        for (JRadioButton x: options) {
        	if (x.isSelected()) {
        		parameters.setActCol(x.getText());
        		break;
        	}
        }  
	}
