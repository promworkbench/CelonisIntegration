...
public class UploadEventLogTimeColumnDialog {
	public UploadEventLogTimeColumnDialog(UIPluginContext context, final UploadEventLogParameter parameters, XLog log) {
        ...
        int w = JOptionPane.showConfirmDialog(null, myPanel,
                "Choose the timestamp column",JOptionPane.OK_CANCEL_OPTION);

        context.getFutureResult(0).setLabel("Timestamp column");
        for (JRadioButton x: options) {
        	if (x.isSelected()) {
        		parameters.setTimeCol(x.getText());
        		break;
        	}
        }  
	}
